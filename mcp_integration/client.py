...
class EnhancedMCPClient:
    ...
    async def connect_to_server(self):
        ...
        # 确保正确解包返回值
        if hasattr(self.stdio_transport, "__iter__") and not isinstance(
            self.stdio_transport, str
        ):
            try:
    ...
    async def anti_bot_protection(self):
        ...
        if time_since_last_call < self.min_call_interval:
            ...
            print(
                f"🛡️ 反爬虫保护：等待 {wait_time:.1f} 秒... / Anti-bot protection: Waiting {wait_time:.1f} seconds..."
            )
    ...
    async def process_query(self, query: str) -> str:
        ...
        print(
            f"🔧 可用工具: {[tool.name for tool in response.tools]} / Available tools: {[tool.name for tool in response.tools]}"
        )
        ...
        if content.finish_reason == "tool_calls":
            ...
            print(
                f"📊 工具执行结果长度: {len(tool_content)} 字符 / Tool execution result length: {len(tool_content)} characters"
            )
    ...
    async def chat_loop(self):
        ...
        print(
            "💡 现在你可以询问任何需要实时信息的问题 / You can now ask any questions that require real-time information"
        )
        print(
            "🔍 系统会自动使用Google搜索来获取最新信息 / The system will automatically use Google Search to get the latest information"
        )
