...
def _detect_locale_timezone(user_locale: Optional[str] = None) -> Tuple[str, str]:
    ...
    try:
        loc = (
            user_locale or pylocale.getdefaultlocale()[0] or os.getenv("LANG", "en-US")
        )
    except Exception:
    ...
    try:
        ...
        tzname = (
            getattr(tz, "key", None) or getattr(tz, "zone", None) or tz.tzname(None)
        )
        if not tzname:
            tzname = "UTC"
...
class BrowserManager:
    ...
    async def launch_browser(
        ...
        logger.info(
            f"Starting persistent browser context headless={headless} user_data={self.user_data_dir}"
        )
        ...
        viewport = {
            "width": max(800, base_viewport.get("width", 1920) + jitter_x),
        ...
        try:
            ...
            err_text = str(e)
            if (
                "expected boolean" in err_text
                or "got string" in err_text
                or "headless: expected" in err_text
            ):
                logger.debug(
                    'Playwright persistent context does not accept headless="new"; falling back to boolean headless'
                )
    ...
    async def _apply_stealth_context_scripts(
        self, context: BrowserContext, user_agent: str, hw: Dict[str, int]
    ) -> None:
    ...
    async def create_context(
        self, browser_or_context, saved_state: SavedState, state_file: str, locale: str
    ) -> BrowserContext:
        ...
        if hasattr(browser_or_context, "new_page") and not hasattr(
            browser_or_context, "new_context"
        ):
        ...
        await self._apply_stealth_context_scripts(
            context, context_options.get("user_agent", ""), _hardware_profile()
        )
        return context
    ...
    async def save_browser_state(
        ...
        try:
            ...
            try:
                fingerprint_data = {
                    "fingerprint": asdict(saved_state.fingerprint)
                    if saved_state.fingerprint
                    else None,
                ...
                logger.error(
                    f"Error while saving fingerprint configuration: {fingerprint_error}"
                )
    ...
    def load_saved_state(
        self, state_file: str
    ) -> tuple[Optional[str], SavedState, str]:
        from common.types import SavedState, FingerprintConfig
        ...
        if Path(state_file).exists():
            ...
            logger.info(
                f"Found browser state file; will use saved state to avoid anti-bot detection: {state_file}"
            )
            ...
            logger.info(
                f"No browser state file found; will create a new browser session and fingerprint: {state_file}"
            )
    ...
    async def human_type(
        ...
        for ch in text:
            ...
            try:
                ...
                await page.evaluate(
                    "(sel, c) => { document.querySelector(sel).value += c }",
                    selector,
                    ch,
                )
