...
class HtmlExtractor:
    ...
    async def extract_html(
        ...
        async with async_playwright() as p:
            ...
            logger.info(
                "浏览器已成功启动!  (Browser started successfully!)"
            ...
            try:
                ...
                if saved_state.google_domain:
                    ...
                    logger.info(
                        f"使用保存的Google域名: {selected_domain}  (Using saved Google domain: {selected_domain})"
                    )
                else:
                    import random
                    ...
                    logger.info(
                        f"随机选择Google域名: {selected_domain}  (Randomly selected Google domain: {selected_domain})"
                    )

                logger.info(
                    "正在访问Google搜索页面...  (Navigating to Google search page)"
                ...
                logger.info(
                    "正在等待搜索结果页面加载完成...  (Waiting for search results page to load)"
                )
                ...
                final_url = page.url
                logger.info(
                    f"搜索结果页面已加载，准备提取HTML: {final_url}  (Search results page loaded; preparing to extract HTML: {final_url})"
                )  # Search results page loaded; preparing to extract HTML

                # 添加额外的等待时间，确保页面完全加载和稳定
                logger.info(
                    "等待页面稳定...  (Waiting for the page to stabilize)"
                )  # Waiting for the page to stabilize...
                await page.wait_for_timeout(
                    1000
                )  # 等待1秒，让页面完全稳定  # Wait 1s to ensure stability
                ...
                logger.info(
                    f"成功获取并清理页面HTML内容: originalLength={len(full_html)}, cleanedLength={len(html)}  (Successfully fetched and cleaned page HTML; originalLength={len(full_html)}, cleanedLength={len(html)})"
                )  # Successfully fetched and cleaned page HTML
                ...
                if save_to_file:
                    ...
                    if not output_path:
                        ...
                        if output_path.is_dir() or not output_path.suffix:
                            ...
                            sanitized_query = re.sub(r"[^a-zA-Z0-9]", "_", query)[:50]
                            output_path = (
                                output_dir / f"{sanitized_query}-{timestamp}.html"
                            )
                    ...
                    logger.info(
                        f"清理后的HTML内容已保存到文件: {output_path}  (Cleaned HTML saved to file: {output_path})"
                    )  # Cleaned HTML saved to file
                    ...
                    # 截取整个页面的截图
                    logger.info(
                        "正在截取网页截图...  (Capturing a screenshot of the page)"
                    ...
                    logger.info(
                        f"网页截图已保存: {screenshot_file_path}  (Screenshot saved: {screenshot_file_path})"
                    )  # Screenshot saved
                ...
                try:
                    ...
                    if not no_save_state:
                        logger.info(
                            f"正在保存浏览器状态: {state_file}  (Saving browser state: {state_file})"
                        )
                        ...
                        logger.info(
                            "浏览器状态保存成功!  (Browser storage state saved successfully)"
                        )  # Browser storage state saved successfully
                        ...
                        try:
                            ...
                            with open(fingerprint_file, "w", encoding="utf-8") as f:
                                json.dump(
                                    fingerprint_data, f, indent=2, ensure_ascii=False
                                )
                            logger.info(
                                f"指纹配置已保存: {fingerprint_file}  (Fingerprint configuration saved: {fingerprint_file})"
                            )  # Fingerprint configuration saved
                        except Exception as fingerprint_error:
                            logger.error(
                                f"保存指纹配置时发生错误: {fingerprint_error}  (Error while saving fingerprint configuration: {fingerprint_error})"
                            )
                    else:
                        logger.info(
                            "根据用户设置，不保存浏览器状态  (Not saving browser state per user setting)"
                        )
                except Exception as error:
                    logger.error(
                        f"保存浏览器状态时发生错误: {error}  (Error while saving browser state: {error})"
                    )
                ...
                logger.error(
                    f"获取页面HTML过程中发生错误: {error}  (Error occurred while fetching page HTML: {error})"
                )  # Error occurred while fetching page HTML
